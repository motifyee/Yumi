...
class ReviewState with _$ReviewState {
  ...
  factory ReviewState.initial() {
    return ReviewState(pagination: PaginatedData<Review>(data: []));
  }

  factory ReviewState.fromJson(Map<String, dynamic> json) => _$ReviewStateFromJson(json);
}
...
class ReviewCubit extends Cubit<ReviewState> {
  ...
  getReviews({required String chefID, bool isMyReviews = false}) async {
    ...
    final task = await GetAllReviews().call(GetAllReviewsParams(chefId: chefID, paginatedData: state.pagination, loginCustomer: isMyReviews, queryParameters: {}));
    task.fold((l) => GlobalContext().snackBar(l.error ?? S.current.apiError), (r) => emit(state.copyWith(pagination: r)));
  }

  Future<bool> updateReviews({required Review review, String? chefId, String? driverId, Map<String, dynamic>? queryParameters}) async {
    final task = await UpdateReview().call(UpdateReviewParams(review: review, chefId: chefId, driverId: driverId, queryParameters: queryParameters));
