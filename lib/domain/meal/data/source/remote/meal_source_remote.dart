...
class MealSourceRemote extends MealSource {
  ...
  Future<String> createMeal({required Meal meal}) async {
    Response res = await DioClient.simpleDio().post(ApiKeys.getApiKeyString(apiKey: ApiKeys.meal), data: meal.toJson());
    return res.data;
  }

  @override
  Future<Meal> deleteMeal({required Meal meal}) async {
    await DioClient.simpleDio().put('${ApiKeys.getApiKeyString(apiKey: ApiKeys.meal)}/${meal.id}/delete', data: {"is_Deleted": true});
    return meal;
  }
  ...
  Future<PaginatedData<Meal>> getFavoriteMeals({required PaginatedData<Meal> pagination}) async {
    ...
    List<Meal> data = res.data['data'].map<Meal>((e) => Meal.fromJson({...e, ...e['meal'], 'id': e['productId'], 'isFavoritProduct': true})).toList();
  ...
  Future<Meal> getMealById({required int mealId}) async {
    Response res = await DioClient.simpleDio().get(ApiKeys.actionApiKeyString(id: mealId.toString(), apiKey: ApiKeys.getMealById));
  ...
  Future<PaginatedData<Meal>> getMeals({required PaginatedData<Meal> pagination, double? lat, double? long, bool? isPreorder = false}) async {
    ...
    List<Meal> data = res.data['data'].map<Meal>((e) => Meal.fromJson({...e, ...e['meal'], 'id': e['productId']})).toList();
  ...
  Future<Meal> addMealToFavorite({required Meal meal}) async {
    Response res = await DioClient.simpleDio().post(ApiKeys.favoriteMeals, data: {'code': CodeGenerator.getRandomCode()}, queryParameters: {'productId': meal.id});
  ...
  Future<Meal> removeMealToFavorite({required Meal meal}) async {
    Response res = await DioClient.simpleDio().delete(ApiKeys.favoriteMeals, queryParameters: {'productId': meal.id});
  ...
  Future<String> updateMeal({required Meal meal}) async {
    Response res = await DioClient.simpleDio().put('${ApiKeys.getApiKeyString(apiKey: ApiKeys.meal)}/${meal.id}', data: meal.toJson());
    return res.data;
  }
}
