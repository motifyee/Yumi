...
class BasketRemoteRepo implements BasketRepo {
  ...
  TaskEither<Failure, Response> deleteBasket({required Basket basket}) =>
      TaskEither.tryCatch(
          () => basketSource.deleteBasket(basket: basket),
          (error, stackTrace) => ServerFailure(
              (error as DioException).response?.data['message'].toString()));
  ...
  TaskEither<Failure, Basket?> getBaskets({Map<String, dynamic>? pagination}) =>
      TaskEither.tryCatch(
          () => basketSource.getBaskets(pagination: pagination),
          (error, stackTrace) => ServerFailure(
              (error as DioException).response?.data['message'].toString()));
  ...
  TaskEither<Failure, Basket> updateBasket({required Basket basket}) =>
      TaskEither.tryCatch(
          () => basketSource.updateBasket(basket: basket),
          (error, stackTrace) => ServerFailure(
              (error as DioException).response?.data['message'].toString()));
  ...
  TaskEither<Failure, Basket> checkVoucherBasket(
      ...
      TaskEither.tryCatch(
          () =>
              basketSource.checkVoucherBasket(basket: basket, voucher: voucher),
          (error, stackTrace) => ServerFailure(
              (error as DioException).response?.data['message'].toString()));
}
